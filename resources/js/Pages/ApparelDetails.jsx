...
const ApparelDetails = ({
        ...
        if (apparelImages.length > 0) {
            return (
                <>
                    {liked == true ? (
                        <Link
                            href={route("like_apparel", { id })}
                            method="post"
                            as="button"
                        >
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                className="absolute w-8 top-52 left-1/4"
                                viewBox="0 0 512 512"
                            >
                                <path
                                    fill="#ff0000"
    ...
    const size = apparelSizes.map((size) => {
        if (size.stock > 0) {
            return (
                <ul key={size.id}>
                    <li key={size.id}>
                        Size : {size.size} Stock: {size.stock} pairs
                    </li>
                </ul>
            );
        }
    ...
    const sizeInput =
        sizes.length > 0 ? (
            ...
            <option disabled className="ms-auto">
                No Size Available
            </option>
        );
                ...
                <form onSubmit={handleSubmit}>
                    <ul className="h-32 ps-4 mt-2" key={apparel.id}>
                        ...
                        <li className="flex items-center mt-3 w-2/4">
                            ...
                            {sizeInput}
                            {errors.id_size && (
                                <p className="text-red-500">{errors.id_size}</p>
                            )}
                        </li>
                        <li className="flex items-center mt-3 w-2/4">
                        ...
                        {sizes.length > 0 ? (
                            <li className=" mt-12 w-32">
                                <button
                                    type="submit"
                                    className="secondary-color text-white px-4 py-2"
                                >
                                    Order Now!
                                </button>
                            </li>
                ...
                <div className="details ms-8">
                    ...
                    <h3>
                        ...
                        {apparelSizes.length > 0 ? (
                            ...
                            <p className="text-red-400">No size available</p>
                        )}
                    </h3>
                    {auth.user ? (
                        <button
                            onClick={handleModalOpen}
                            className="mt-3 px-5 py-2 secondary-color text-white"
                        >
                            Order Now
                        </button>
                    ) : (
